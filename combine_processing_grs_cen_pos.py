...
def fit_gaussian_2d(image, contour):
    ...
    initial_guess = (w//2, h//2, 1, 1, roi.max())
    try:
        popt, _ = curve_fit(lambda data, x0, y0, sigma_x, sigma_y, A: 
                           gaussian_2d(data[0], data[1], x0, y0, sigma_x, sigma_y, A),
                           (X, Y), Z, p0=initial_guess, maxfev=5000)
        return (int(popt[0] + x), int(popt[1] + y), popt[4])  # (x, y, brightness)
    except:
        print("Gaussian fit did not converge")
        return (x + w//2, y + h//2, roi.max())  # Fallback to bounding box center
...
def highlight_stars(image, reference_stars):
    ...
    if reference_stars:
        ...
        cv2.circle(image, reference_stars[0]['pos'], 7, (0, 0, 255), -1)
...
if __name__ == "__main__":
    ...
    dataset = StarDataset(
        processed_dir="/media/student/B076126976123098/my_data/SiT/dataset_sky/new_all_star_processed/processed_images",
